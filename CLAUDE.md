...
- Menu data source of truth is `backend/scripts/menu_seed.json`, generated by
  `scripts/build_seed.py` — transcribed from photos of the current St. Pete
  printed menu (July 2026). Categories noted "Prices under review" (classic
  Sushi Rolls, Naruto, Sushi Bar Special, Beverages) carry prices from
  `scripts/legacy_menu.json` until the owner confirms; edit build_seed.py and
  rerun it + seed.py to change menu data.
- Photos in `frontend/public/assets/img/` are real photos of this location
  (storefront + dishes) sourced from its public listings — replace with
  owner-provided originals when available.
