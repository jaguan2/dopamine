# Dopamine

Full-stack website and online ordering for **O.E.C. Japanese Express**,
2438 66th St North, St. Petersburg, FL 33710 · 727-345-4088. Closed Tuesdays.

The name comes from the Korean idea of a *dopamine site* (도파민 사이트): a
storefront people browse, fill a cart on, and enjoy for its own sake without
ever checking out. That describes this build exactly. Every part of the
ordering flow is real and functional except the payment step, which is
settled in person at pickup or delivery.

Customers browse the full menu (288 dishes), build a cart, and place a
pickup or delivery order. Staff work the queue on a live kitchen dashboard.

## Stack

| Layer    | Tech                                                             |
|----------|------------------------------------------------------------------|
| Frontend | React 19 (Vite, JSX), react-router-dom v7, axios, plus a custom CSS design system (washi/sumi/vermilion, no UI framework) |
| Backend  | Flask + Flask-SQLAlchemy + Flask-CORS                            |
| Database | SQLite by default; set `SQLALCHEMY_DATABASE_URI` for Postgres    |

## Quick start

Backend (port 5000):

```
cd backend
python -m venv venv
venv\Scripts\pip install -r requirements.txt
venv\Scripts\python scripts\seed.py     # builds tables + loads the menu
venv\Scripts\python main.py
```

Frontend (port 5173, proxies `/api` to 5000):

```
cd frontend
npm install
npm run dev
```

Open http://localhost:5173. The kitchen dashboard is at `/kitchen`
(default key `oec-kitchen`; change via `KITCHEN_KEY` in `backend/.env`).

Tests (20 API-contract tests, throwaway DB):

```
cd backend
venv\Scripts\python -m pytest
```

## How it fits together

```
frontend/src/
  pages/        Home, Menu (search + add to cart), Checkout,
                OrderStatus (/order/:code, polls), Kitchen (/kitchen),
                NotFound (catch-all, see the routing note below)
  components/   Header (cart badge), Footer, CartDrawer, CartLine, BackToTop
  contexts/     CartContext, cart lines held in localStorage
  styles/       site.css, the whole design system

backend/
  config_db.py  app + db + constants (TAX_RATE, DELIVERY_MINIMUM, KITCHEN_KEY)
  main.py       blueprint registration + entrypoint
  models/       Location · Category · MenuItem · PriceOption · Order · OrderItem
  routes/       menu_routes (GET /api/menu, /api/locations)
                order_routes (POST /api/orders, GET /api/orders/<code>,
                              /api/kitchen/* behind X-Kitchen-Key)
  utils/        rate_limit.py, in-process sliding-window limiter
  scripts/      menu_seed.json + seed.py (idempotent: reseeding the menu
                never touches placed orders)
```

Domain rules worth knowing:

- **Money is integer cents everywhere.** Formatting happens only at render.
- **The server never trusts client prices.** Orders are repriced from
  `PRICE_OPTION` rows at POST time and snapshot name/price per line, so
  menu edits never rewrite order history.
- Order status flow: `received → confirmed → ready → completed`, or
  `cancelled` from any non-terminal state. The kitchen can only step forward
  or cancel; an illegal jump (say, completed back to received) returns 409.
  The status page polls every 10 s, the kitchen every 8 s.
- Delivery has a $15.00 pre-tax minimum (enforced client- and server-side);
  tax is 7% (FL 6% + Pinellas County 1%).
- `POST /api/orders` is rate limited to 10 per minute per client so the queue
  can't be flooded. The limiter is in-process (`utils/rate_limit.py`); if the
  app is ever run with multiple workers, move it to a shared store, and add
  werkzeug's `ProxyFix` when it runs behind a reverse proxy.
- **Every SPA route needs a match.** The dev server and any static host serve
  `index.html` for all paths, so a URL with no matching `<Route>` renders a
  bare header and footer with an empty `<main>`. `App.jsx` keeps a catch-all
  `*` route to `NotFound` and redirects `/index.html` to `/`.

## Menu data

`backend/scripts/menu_seed.json` holds the menu (29 categories / 288 items
with descriptions, spicy flags, Japanese labels). It is generated by
`scripts/build_seed.py` from `scripts/menupages_menu.json`, which is
**OEC's own online-ordering feed** (Grubhub restaurant 560075, the same menu
behind oecjapaneseexpress.dine.online), captured via Menupages in July 2026.
Reseed any time with `python scripts/seed.py`.

These are the prices OEC charges for online orders, deliberately a little
above the printed in-store menu (for example, hibachi chicken lunch is $9.45
online vs $8.95 in store). Two in-store-only categories (Poke Bowl, Sake) are
appended in `build_seed.py` from the printed menu, since the online feed
omits them.

## Before a real launch

- Payment (Stripe), if online payment is ever wanted. The checkout is built
  to slot it in after the order summary step. Leaving it out is the point of
  the name, so this is a choice rather than a gap.
- Order notifications the kitchen will actually see (printer, SMS, email).
  The dashboard covers a demo, but a busy kitchen needs a push channel.
- Photography. The site is deliberately photo-free (typographic design)
  until the owner provides quality originals; drop them into
  `frontend/public/assets/img/` and re-add `<img>` blocks where wanted.
- Real hosting: build the frontend (`npm run build`) and serve `dist/`
  behind the same origin as Flask, or use any static host plus API host pair.
